...
function printOutput(num) {
	if (num === ""){
    	...
    	document.getElementById('alloutput-value').innerText=getFormattedNumber(num);
    }
}
        ...
        if (this.id=='backspace'){
            ...
            if(alloutput==''&&allhistory!=''){
                if(isNaN(allhistory[allhistory.length-1])){
                    allhistory=allhistory.substring(0,history.lenght-1);
                }
            }
            if(alloutput!='' || allhistory!=''){
                alloutput= alloutput==''?
                alloutput:reverseNumberFormat(alloutput);
